...
def run_professional_backtest(start_yr, model_choice, t_costs_bps, stop_loss_pct, recovery_sigma, _force_sync=False):
    ...
    for ticker in assets:
        try:
            ...
            if any(opt in model_choice for opt in ["Option I", "Option J", "Option K"]):
                ...
                preds = eng.predict_series(X[m_oos]) * conf_vec[-len(X[m_oos]):]
    ...
    # --- EXECUTION LOOP WITH DYNAMIC RISK GATE ---
    ...
    for d in common_idx:
        ...
        # Scale z-score to a 0-1 conviction for the UI
...
# --- EXECUTE ---
with st.spinner("Calculating Multi-Model Backtest..."):
...
st.dataframe(audit_df.style.format({'Return': '{:.2%}', 'Z-Score': '{:.2f}'}), use_container_width=True)
